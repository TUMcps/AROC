...
%
% Description:
%       Simulate a trajectories of a linear closed-loop system 
%       controlled with a terminal region controller corresponding to a 
%       terminal region computed with the zonotope approach for linear 
%       systems for random initial points inside the terminal region and 
%       randomly drawn disturbance values.
%
% Input Arguments:
%
...
%------------------------------------------------------------------    
    ...
    for i = 1:Npoints
        ...
        V{i} = zeros(dim(obj.set),floor(tFinal/obj.dt));
        
        if ~isempty(obj.V)
           for j = 1:floor(tFinal/obj.dt)
              V{i}(:,j) = randPoint(obj.V); 
           end
        end
    end
