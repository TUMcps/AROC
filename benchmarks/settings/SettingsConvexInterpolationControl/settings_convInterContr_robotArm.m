function Opts = settings_convInterContr_robotArm()
% SETTINGS_CONVINTERCONTR_ROBOTARM - algorithm settings for the robot arm
%                                    benchmark
%
% Syntax:
...
%       Control Algorithm for the robot arm benchmark.
%
% Output Arguments:
%
...
%
% See Also:
%       convexInterpolationControl, robotArm
%
%------------------------------------------------------------------
...
%------------------------------------------------------------------
    ...
    Opts.refTraj.Q = eye(4)./(0.05^2);             
    Opts.refTraj.R = diag([1;1]./([3;1].^2)); 
    
    % weighting matrices for the cost function (vertex trajectories)
    Opts.Q = diag([10,10,1,1]);             
    Opts.R = diag([0;0]); 
