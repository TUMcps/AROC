function f = dynamicsClosedLoopQuadraticMeasErr(x,w,p,nx,dynamics)
% DYNAMICSCLOSEDLOOPQUADRATICMEASERR - system dynamics for the closed-loop 
%                                      system controlled with the quadratic 
%                                      control law
%
% Syntax:
%       f = DYNAMICSCLOSEDLOOPQUADRATICMEASERR(t,x,w,p,nx,dynamics)
%
% Description:
...
%------------------------------------------------------------------
    ...
    index = 1; 
    for i = 1:nu
        A{i} = diag(p(index:index+nx-1));
        index = index + nx;
        b{i} = p(index:index+nx-1);
        index = index + nx;
        o{i} = p(index);
        index = index + 1;
    end
